...
export class LauncherApp extends EventEmitter {
  ...
  get userAgent() {
    ...
    return `chaldstudio/chald-launcher/${version} (chaldlauncher.app)`
  }
  ...
  readonly server: Server = createServer((req, res) => {
    this.protocol.handle({
      method: req.method,
      url: new URL(req.url ?? '/', 'chald://launcher'),
      headers: req.headers,
      body: req,
    }).then((resp) => {
  ...
  protected async setup() {
    ...
    // register chald protocol
    if (!this.host.isDefaultProtocolClient('chald')) {
      const result = this.host.setAsDefaultProtocolClient('chald')
      if (result) {
  ...
  protected async getStartupUrl(): Promise<string | undefined> {
    if (!IS_DEV && process.platform === 'win32') {
      ...
      if (process.argv.length > 1) {
        ...
        const protocolOption = process.argv.find(a => a.startsWith('chald://'))
        if (protocolOption) {
