...
export const pluginSetup: LauncherAppPlugin = async (app) => {
  ...
  app.controller.handle('preset', async () => {
    ...
    // onboarding default so the wizard never points at the real ~/.minecraft.
    const defaultPath = process.env.XMCL_E2E_GAME_DATA || join(app.host.getPath('home'), 'chaldl')
    const getPath = (driveSymbol: string) => {
      ...
      return join(driveSymbol, 'chaldl')
    }
