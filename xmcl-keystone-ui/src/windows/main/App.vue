<template>
  <v-app v-if="!showSetup" class="h-full max-h-screen overflow-hidden" :class="{ 'dark': isDark }">
    ...
    <AppChaldStudioUpdateDialog ref="chaldUpdateDialog" @installed="onChaldInstalled" />
...
import AppChaldStudioUpdateDialog from '@/views/AppChaldStudioUpdateDialog.vue'
...
const chaldUpdateDialog = ref<InstanceType<typeof AppChaldStudioUpdateDialog>>()
...
const onReady = async (data: any) => {
  ...
  tutor.start()
  // Проверяем обновление сборки после завершения установки
  nextTick(() => chaldUpdateDialog.value?.checkForUpdate())
}
...
// ChaldStudio auto-update check on launch
const onChaldInstalled = (instancePath: string) => {
  notify({
    level: 'success',
    title: 'Сборка ChaldStudio установлена!',
    body: 'Переключитесь на неё в списке инстансов',
  })
}

onMounted(() => {
  // Если это не первый запуск (без setup), проверяем обновление сразу
  if (!showSetup.value) {
    nextTick(() => chaldUpdateDialog.value?.checkForUpdate())
  }
})
