<template>
  <div class="sidebar-instances">
    ...
    />
    <AppSideBarItem
      id="chaldstudio-download-button"
      v-shared-tooltip.right="() => 'Скачать сборку ChaldStudio'"
      :aria-label="'Скачать сборку ChaldStudio'"
      icon="download"
      clickable
      class="chaldstudio-button"
      :loading="chaldDownloading"
      @click="onDownloadChaldStudio"
    />
  </div>
</template>
...
import { useChaldStudioModpack } from '@/composables/chaldStudio'
...
const { downloading: chaldDownloading, downloadAndInstall } = useChaldStudioModpack()
const onDownloadChaldStudio = () => {
  downloadAndInstall()
}
