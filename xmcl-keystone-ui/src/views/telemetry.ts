...
const appInsights = new ApplicationInsights({
  config: {
    ...
    // Telemetry is hard-disabled for ChaldLauncher: this key/endpoint
    // belongs to upstream XMCL, not us, and we don't want to ship any
    // user data there. `disableTelemetry: true` is the SDK's own kill
    // switch — the instance still initializes (so any `appInsights.*`
    // calls elsewhere in the app stay safe no-ops), but nothing is ever
    // queued or sent. There is intentionally no user-facing setting to
    // turn this back on.
